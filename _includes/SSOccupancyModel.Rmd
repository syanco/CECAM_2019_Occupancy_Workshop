...
Before we get started we need to load in the R packages needed for this coding tutorial. Make sure you install these packages first before you try to load them. 
...
Looks like we could reduce the number of our visits and still do a decent job of estimating both parameters! Visiting 100 meadows is still a lot of work so can we reduce that as well? We will use the same code as above but modify it to step through number of sites.
...
We could probably reduce the number of sites we visit but probably not by more than half. 
